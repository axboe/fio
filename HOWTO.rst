...
.. option:: verify_backlog=int
	...
	write only N blocks before verifying these blocks. Note that for mixed
	workloads ``rw=readwrite`` and ``rw=randrw``, reads generated by the
	workload itself will not verify data, even when addressing blocks
	written by the workload - only reads issued by the ``verify_backlog``
	logic will verify the data read.
