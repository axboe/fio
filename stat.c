			     ...
			     int ddir, struct buf_output *out)
{
	...
	if (ddir == DDIR_READ && calc_lat(&ts->clat_high_prio_stat, &min, &max, &mean, &dev))
	...
	if (ts->clat_percentiles || ts->lat_percentiles) {
		...
		if (ddir == DDIR_READ && ts->clat_high_prio_stat.samples > 0) {
			sprintf(prio_name, "high prio (%.2f%%) %s",
					100. * (double) ts->clat_high_prio_stat.samples / (double) samples,
					name);
			show_clat_percentiles(ts->io_u_plat_high_prio,
						ts->clat_high_prio_stat.samples,
						ts->percentile_list,
		...
		struct group_run_stats *rs, int ddir, struct json_object *parent)
{
	...
	if (ts->clat_high_prio_stat.samples > 0) {
		/* START OF HIGH PRIO CLAT */

	    if (ddir == DDIR_READ) {
		    if (!calc_lat(&ts->clat_high_prio_stat, &min, &max, &mean, &dev)) {
				min = max = 0;
				mean = dev = 0.0;
			}
			tmp_object = json_create_object();
			...
			if (ts->clat_percentiles) {
				len = calc_clat_percentiles(ts->io_u_plat_high_prio,
							ts->clat_high_prio_stat.samples,
							ts->percentile_list, &ovals, &maxv,
							&minv);
			} else
	    ...
	    if (ddir == DDIR_READ) {
		    if (!calc_lat(&ts->clat_prio_stat, &min, &max, &mean, &dev)) {
				min = max = 0;
				mean = dev = 0.0;
			}
			tmp_object = json_create_object();
			...
			if (ts->clat_percentiles) {
				len = calc_clat_percentiles(ts->io_u_plat_prio,
							ts->clat_prio_stat.samples,
							ts->percentile_list, &ovals, &maxv,
							&minv);
			} else
		      ...
		      bool first)
{
	...
	for (l = 0; l < DDIR_RWDIR_CNT; l++) {
		if (!dst->unified_rw_rep) {
			...
			if (l == DDIR_READ) {
				sum_stat(&dst->clat_high_prio_stat, &src->clat_high_prio_stat, first);
				sum_stat(&dst->clat_prio_stat, &src->clat_prio_stat, first);
			}
	...
	for (k = 0; k < DDIR_RWDIR_CNT; k++) {
		...
		for (m = 0; m < FIO_IO_U_PLAT_NR; m++) {
			if (!dst->unified_rw_rep) {
				...
				if (k == DDIR_READ) {
					dst->io_u_plat_high_prio[m] += src->io_u_plat_high_prio[m];
					dst->io_u_plat_prio[m] += src->io_u_plat_prio[m];
				}
			}
			else {
...
void init_thread_stat(struct thread_stat *ts)
{
	...
	ts->clat_high_prio_stat.min_val = -1UL;
	ts->clat_prio_stat.min_val = -1UL;
...
static void __add_log_sample(struct io_log *iolog, union io_sample_data data,
			     ...
			     unsigned long t, uint64_t offset, unsigned int priority_bit)
{
	struct io_logs *cur_log;
...
void reset_io_stats(struct thread_data *td)
{
	...
	reset_io_stat(&ts->clat_high_prio_stat);
	reset_io_stat(&ts->clat_prio_stat);
	for (i = 0; i < DDIR_RWDIR_CNT; i++) {
		...
		for (j = 0; j < FIO_IO_U_PLAT_NR; j++) {
			ts->io_u_plat[i][j] = 0;
			if (i == DDIR_READ) {
				ts->io_u_plat_high_prio[j] = 0;
				ts->io_u_plat_prio[j] = 0;
			}
			if (!i)
...
static void __add_stat_to_log(struct io_log *iolog, enum fio_ddir ddir,
			      unsigned long elapsed, bool log_max, bool priority_bit)
{
	/*
...
static void _add_stat_to_log(struct io_log *iolog, unsigned long elapsed,
			     bool log_max, unsigned int priority_bit)
{
	int ddir;
...
static unsigned long add_log_sample(struct thread_data *td,
				    ...
				    union io_sample_data data,
				    enum fio_ddir ddir, unsigned long long bs,
				    uint64_t offset, bool priority_bit)
{
				...
				unsigned long long nsec, enum fio_ddir ddir)
{
	...
	if (!ts->priority_bit && ddir == DDIR_READ) {
		ts->io_u_plat_prio[idx]++;
	}

	if (ts->priority_bit && ddir == DDIR_READ) {
		ts->io_u_plat_high_prio[idx]++;
		ts->priority_bit = 0;
	}
}
		     ...
		     uint64_t offset, bool priority_bit)
{
	...
	if (priority_bit && ddir == DDIR_READ) {
		add_stat_sample(&ts->clat_high_prio_stat, nsec);
	}

	if (!priority_bit && ddir == DDIR_READ) {
		add_stat_sample(&ts->clat_prio_stat, nsec);
	}
		   ...
		   unsigned int bytes, unsigned long long spent)
{
	...
	if (td->bw_log)
		add_log_sample(td, td->bw_log, sample_val(rate), io_u->ddir,
			       bytes, io_u->offset, io_u->priority_bit);
