			  ...
			  uint64_t align_bs)
{
	...
	if ((spr_info->cache_sz != 0) && (spr_info->cache_sz > region_sz)) {
		log_err("fio: sprandom: spr_cs [%"PRIu64"] must be smaller than region_sz [%"PRIu64"] which means [%"PRIu64"] regions allowed",
			spr_info->cache_sz,
		    region_sz,
	...
	if (spr_info->cache_sz != 0) {
		/* Need 2x size to be safe since we wait to invalidate until after next region */
		invalid_capacity = estimate_inv_capacity(region_write_count, validity_dist[0]) * 2;
	} else {
		invalid_capacity = estimate_inv_capacity(region_write_count, validity_dist[0]);
	}
	...
	dprint(FD_SPRANDOM, "  cache_size:         %"PRIu64": %s\n",
		spr_info->cache_sz,
		bytes2str_simple(bytes2str_buf, sizeof(bytes2str_buf), spr_info->cache_sz));
...
int sprandom_get_next_offset(struct sprandom_info *info, struct fio_file *f, uint64_t *b)
{
	...
	if (info->cache_sz == 0) {
		...
		/* replay invalidation at start of next region prior to moving to new region */
	...
	if (info->writes_remaining == 0) {
		...
		if (info->cache_sz != 0) {
			...
			/* replay invalidation for previous region at end of this region to avoid invalidations
			 * hitting the defined cache.
			 */
			if (pcb_pop(info->invalid_buf, &offset)) {
				sprandom_add_with_probability(info, offset,  phase ^ 1);
				dprint(FD_SPRANDOM, "Cache Defer Write %"PRIu64" over %d\n",
					offset, info->current_region);
				goto out;
			}
		}
	...
	if (lfsr_next(&f->lfsr, &offset)) {
		...
		if (info->cache_sz != 0) {
			...
			/* Since we defer invalidation to the end of next region we need to take into account end of lfsr case */
			if (pcb_pop(info->invalid_buf, &offset)) {
				dprint(FD_SPRANDOM, "lfsr cache exit Write %"PRIu64" over %d\n",
					offset, info->current_region);
				goto out;
			}
		}
